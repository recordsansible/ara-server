...
class StatsTestCase(APITestCase):
    def test_stats_factory(self):
        stats = factories.StatsFactory(
            changed=2, failed=1, ok=3, skipped=2, unreachable=1
        )
    ...
    def test_stats_serializer(self):
        ...
        serializer = serializers.StatsSerializer(
            data=dict(playbook=playbook.id, host=host.id, ok=9001)
        )
        serializer.is_valid()
    ...
    def test_create_stats(self):
        ...
        request = self.client.post(
            "/api/v1/stats", dict(playbook=playbook.id, host=host.id, ok=9001)
        )
