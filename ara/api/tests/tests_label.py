...
class LabelTestCase(APITestCase):
    ...
    def test_label_serializer_compress_description(self):
        serializer = serializers.LabelSerializer(
            data={"name": "compress", "description": factories.LABEL_DESCRIPTION}
        )
        serializer.is_valid()
        label = serializer.save()
        label.refresh_from_db()
        self.assertEqual(
            label.description, utils.compressed_str(factories.LABEL_DESCRIPTION)
        )
    ...
    def test_create_label(self):
        ...
        request = self.client.post(
            "/api/v1/labels",
            {"name": "compress", "description": factories.LABEL_DESCRIPTION},
        )
