...
class ResultTestCase(APITestCase):
    ...
    def test_result_serializer_compress_content(self):
        ...
        self.assertEqual(
            result.content,
            b'x\x9c\xabV*J-.\xcd))V\xb2R\x88VJ\xcb\xcfW\xd2QPJJ,R\x8a\xad\x05\x00~\xf4\x08\xae'
        )

    def test_result_serializer_decompress_content(self):
        result = factories.ResultFactory(
            content=b'x\x9c\xabV*J-.\xcd))V\xb2R\x88VJ\xcb\xcfW\xd2QPJJ,R\x8a\xad\x05\x00~\xf4\x08\xae'
        )
        serializer = serializers.ResultSerializer(instance=result)
        self.assertEqual(serializer.data['content'], {'results': ['foo', 'bar']})
